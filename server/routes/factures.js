...
// GET /api/factures/:id - avec ses lignes
router.get('/:id', (req, res) => {
  const facture = db.prepare(`
    SELECT f.*, c.nom as contact_nom, c.entreprise as contact_entreprise, c.email as contact_email, c.adresse as contact_adresse
...
// POST /api/factures - création avec lignes
router.post('/', (req, res) => {
  ...
  const insertFacture = db.prepare(`
    INSERT INTO factures (numero, type, contact_id, opportunite_id, statut, montant_ht, taux_tva, montant_ttc, date_echeance, owner_id)
    VALUES (?, ?, ?, ?, 'brouillon', ?, ?, ?, ?, ?)
  `);
  ...
  const transaction = db.transaction(() => {
    ...
    for (const ligne of lignes) {
      insertLigne.run(factureId, ligne.designation, ligne.quantite, ligne.prix_unitaire, ligne.quantite * ligne.prix_unitaire);
    }
    return factureId;
  });

  const factureId = transaction();
  logActivite(type === 'facture' ? 'facture_creee' : 'devis_cree', `${numero} créé(e)`, contact_id, req.session.userId);
...
// PUT /api/factures/:id/statut - changement de statut rapide
router.put('/:id/statut', (req, res) => {
  ...
  logActivite('facture_statut', `${existing.numero} marqué(e) "${statut}"`, existing.contact_id, req.session.userId);
...
// GET /api/factures/:id/pdf - génère un PDF imprimable pour la facture/devis
router.get('/:id/pdf', (req, res) => {
  ...
  // Small domain at top-right (no protocol)
  const domain = 'sudicone.com';
  doc.font('Helvetica').fontSize(10).text(domain, doc.page.width - 150, 60, { width: 120, align: 'right', color: '#ffffff' });

  // Meta box (white) on header right
  ...
  doc.fillColor(BLUE).font('Helvetica-Bold').fontSize(12).text(facture.type === 'facture' ? 'FACTURE' : 'DEVIS', infoX + 12, infoY + 8);
  const dateEmission = facture.date_emission ? facture.date_emission.slice(0,10) : new Date().toISOString().slice(0,10);
  doc.font('Helvetica').fontSize(10).fillColor('#333').text(`N° ${facture.numero || ''}`, infoX + 12, infoY + 28);
  ...
  cursorY += 18;
  if (facture.contact_entreprise) { doc.font('Helvetica-Bold').fontSize(11).text(facture.contact_entreprise, 50, cursorY); cursorY += 16; }
  ...
  facture.lignes.forEach((l, idx) => {
    ...
    doc.fillColor('#333').text(l.designation || '-', colX.desc + 2, rowY, { width: colX.qty - colX.desc - 10 });
    doc.text(String(l.quantite || 0), colX.qty, rowY, { width: 40, align: 'right' });
    doc.text(Number(l.prix_unitaire || 0).toLocaleString('fr-FR', { maximumFractionDigits: 0 }) + ' XOF', colX.pu, rowY, { width: 60, align: 'right' });
    doc.text(Number(lineTotal).toLocaleString('fr-FR', { maximumFractionDigits: 0 }) + ' XOF', colX.total, rowY, { width: 60, align: 'right' });
    rowY += 22;
  });
  ...
  doc.font('Helvetica').fontSize(10).fillColor('#333').text('Sous-total HT', totalsX - 20, rowY + 14, { width: 150, align: 'left' });
  doc.text(Number(subtotal).toLocaleString('fr-FR', { maximumFractionDigits: 0 }) + ' XOF', colX.total, rowY + 14, { width: 60, align: 'right' });
  doc.text(`TVA (${tva}%)`, totalsX - 20, rowY + 34, { width: 150, align: 'left' });
  doc.text(Number(montantTva).toLocaleString('fr-FR', { maximumFractionDigits: 0 }) + ' XOF', colX.total, rowY + 34, { width: 60, align: 'right' });
  ...
  doc.font('Helvetica-Bold').fontSize(12).text(Number(totalTtc).toLocaleString('fr-FR', { maximumFractionDigits: 0 }) + ' XOF', boxX, boxY + 8, { width: boxW - 18, align: 'right' });
  ...
  // Footer line and contact (center)
  doc.moveTo(48, doc.page.height - 60).lineTo(doc.page.width - 48, doc.page.height - 60).stroke('#e6e6e6');
  doc.font('Helvetica').fontSize(9).fillColor('#888').text('Contact: 226 04864124 • sudicone226@outlook.fr • sudicone.com', 48, doc.page.height - 48, { align: 'center', width: doc.page.width - 96 });
