...
router.put('/:id', (req, res) => {
  ...
  db.prepare(`
    UPDATE workflows SET titre = ?, description = ?, contact_id = ?, statut = ?,
      date_debut = ?, date_fin_prevue = ?, date_terminee = ? WHERE id = ?
  `).run(
...
router.put('/steps/:stepId', (req, res) => {
  ...
  db.prepare(`
    UPDATE workflow_steps SET titre = ?, description = ?, statut = ?, ordre = ?,
      assigned_to = ?, date_debut = ?, date_fin = ? WHERE id = ?
  `).run(
    titre ?? existing.titre,
    description ?? existing.description,
    statut ?? existing.statut,
...
router.delete('/:id', (req, res) => {
  const existing = db.prepare('SELECT * FROM workflows WHERE id = ?').get(req.params.id);
  if (!existing) return res.status(404).json({ error: 'Workflow introuvable' });
  logActivite('workflow_supprime', `Workflow "${existing.titre}" supprimé`, existing.contact_id, req.session.userId);
